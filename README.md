...
2022.02.23
4. 新增豆瓣详细配置，可以根据豆瓣想看的标签，决定选择策略以及对关键字加权，这个关键字可以是字幕组、压制组、站点名（英文跟pt部分配置一样）。可以根据不同的豆瓣id，选择策略；
5. 新增通知时增加评分显示，来自谁的豆瓣昵称下载；
6. 新增fanart API，下载完成发送通知时，会优先去fanart查找电影封面图，其次TMDB，最后用豆瓣的；
7. 优化下载电影过程，全面加速。增加查找缓存，多PT站点并行搜索；除首次运行任务外，多pt站时处理速度至少是以前版本的十倍；
8. 优化Emby智能刷新功能的媒体库识别；
9. 优化豆瓣多id想看时，重复电影的处理；
10. 修复手动挡下载时，部分资源解析会出错的BUG
