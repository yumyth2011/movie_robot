        ...
        for row in soup.find('table', class_="torrents").findAll('tr', recursive=False)[1:]:
            ...
            for i, item in enumerate(row.findAll('td', recursive=False)[:-1]):
                if i == 0:
                    ...
                    info = item.select('table tr td.embedded')[1]
                    a = info.select_one('div div a[title]')
                    t.name = a['title']
                    t.id = re.findall('id=(\d+)', a['href'])[0]
                    if info.select_one('div div a[href="torrents.php?spstate=2"]'):
                        deadline = info.select_one('div div a[href="torrents.php?spstate=2"] ~ b span[title]')
                        if deadline:
